...
const PhysicsScene = () => {
  ...
  const firstCubeClickHandler = () => {
      firstCubeRef.current.applyTorqueImpulse(
      {x: 100, y: 0, z: 0}
      )
  }
  ...
  return (
    <Physics>
      <Debug />
      <RigidBody ref={firstCubeRef} mass={5} >
        <mesh castShadow position={[1.5, 2.5, 0]} onClick={firstCubeClickHandler}
        scale={3}>
          <boxGeometry />
      ...
      <RigidBody ref={secondCubeRef} >
        <mesh position={[-1.5, 2.5, 0]} onClick={secondCubeClickHandler}>
          <boxGeometry />
          <meshStandardMaterial color="#CC3941" />
        </mesh>
      </RigidBody>
